...
mod tests {
    ...
    #[test]
    pub fn test_set_colaborador() {
        set_context();
        let mut contrato = Contract::default();

        contrato.set_colaborador(DATASET_ID);

        let p = contrato.colaboradores.get(&String::from(CUENTA)).unwrap();

        assert_eq!(p.dataset_id, DATASET_ID);
    }
}
